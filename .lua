...
if eq(time3,tostring(Base64.Decode(res1.data)),true) then
	...
	local Window = library:Window("Bloxburg", "By Rent")
	...
	local Tabs = {
		...
		ServerInvite = Window:Tab("Jailbreak Savages");
	}
	...
	if game:HttpGet("https://bloxburg.sexy/version.php") ~= tostring(ReplicatedStorage.Version.Value) then 
		Window:Notification("Error", {
			Text = "Made by rent Also\nthe script may not work\nand/or be vulnurable :/";
			ConfirmText = "Okay";
		})
	end

	Sections.ServerInvite.Invite:Button("Copy jb savages inv", function()
		setclipboard(game:HttpGet("https://discord.gg/pbw6XT6g"))
	end)
